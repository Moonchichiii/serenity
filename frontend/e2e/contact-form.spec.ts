...
test.describe("navigation", () => {
  ...
  test("gift modal opens from floating button", async ({ page }) => {
    ...
    const giftTrigger = page.getByRole("button", { name: /offer a gift|offrir un cadeau/i });
    await expect(giftTrigger).toBeVisible();
    await giftTrigger.click();

    await expect(page.getByRole("dialog")).toBeVisible();
    await expect(
      page.getByRole("combobox", { name: /choose a treatment|choisir un soin/i }),
    ).toBeVisible();
  });
  ...
  test("contact modal opens from header", async ({ page }) => {
    ...
    await expect(page.getByRole("dialog")).toBeVisible();
    await expect(page.getByLabel(/full name|nom complet/i)).toBeVisible();
  ...
  test("voucher success route responds", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: /processing payment|payment successful|something went wrong/i,
      }),
    ).toBeVisible();
  });
