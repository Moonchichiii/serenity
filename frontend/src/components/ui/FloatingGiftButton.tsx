...
export function FloatingGiftButton() {
    ...
    useEffect(() => {
        cmsAPI.getGlobals().then((data) => {
            setEnabled(data.gift.is_enabled)
            if (data.gift.floating_icon?.url) {
                setIconUrl(data.gift.floating_icon.url)
            }
            setLoaded(true)
        }).catch(() => {
            setEnabled(true)
    ...
    return (
        <AnimatePresence>
            ...
            >
                <div className="absolute inset-0 bg-terracotta-400 rounded-full animate-pulse-warm opacity-30"></div>
                <div className="relative flex items-center justify-center w-16 h-16 bg-white/90 backdrop-blur-sm rounded-full shadow-elevated border-2 border-white/50 transition-all group-hover:border-terracotta-200 overflow-hidden p-3">
                    {iconUrl ? (
                        <img
                            src={iconUrl}
                            alt="Gift"
