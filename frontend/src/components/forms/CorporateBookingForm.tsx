...
type CorporateFormData = {
  ...
  endDate?: string         // optional end of range
  duration?: string        // e.g. "3h", "09:00-17:00"
  onSiteAddress?: string   // location / venue
  notes?: string           // extra info / requests
  budget?: string          // optional budget hint
  services?: string        // free text or comma-separated choices
}
...
export function CorporateBookingForm({
  ...
  const inputPlain =
    'w-full px-4 py-3 rounded-xl border-2 border-sage-200 focus:border-sage-400 focus:ring-2 focus:ring-sage-200 transition-colors'
  ...
  const onSubmit = async (data: CorporateFormData) => {
    ...
    // Build a structured message body (keeps your single endpoint)
    const lines = [
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
      ...
      <div>
        <label className="block text-sm font-medium text-charcoal mb-2" htmlFor="phone">
          {t('corp.form.phone', 'Phone')} <span className="text-xs text-charcoal/80">{t('corp.form.optional', '(optional)')}</span>
        </label>
        <div className="relative">
      ...
      <div className="grid md:grid-cols-3 gap-5">
        ...
        <div>
          ...
          <input id="duration" type="text" className={inputPlain} placeholder="09:00–17:00 or 3h" {...register('duration')} />
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <div>
          ...
          <input
            ...
            placeholder={t('corp.form.services.placeholder', 'Chair massage, 10-min rotations, 2 practitioners…')}
            {...register('services')}
          />
        </div>
        ...
        <div>
          ...
          <input id="budget" type="text" className={inputPlain} placeholder="€500–€1500" {...register('budget')} />
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder={t('corp.form.notes.placeholder', 'Ambiance / space available, parking, access badges, etc.')}
          {...register('notes')}
        />
      </div>
      ...
      <div className="bg-sage-50 rounded-lg p-4 border border-sage-200">
        <p className="text-xs text-charcoal/80 leading-relaxed">
          <span className="font-semibold text-charcoal">{t('corp.form.gdpr.title', 'Privacy notice')}:</span>{' '}
          {t(
            'corp.form.gdpr.text',
