...
export default function GiftVoucherModalScreen() {
  ...
  const title = useMemo(() => {
    if (!giftSettings) return t('gift.title')
    return lang === 'fr' ? giftSettings.modal_title_fr : giftSettings.modal_title_en
  }, [giftSettings, lang, t])

  const body = useMemo(() => {
    if (!giftSettings) return t('gift.subtitle')
    return lang === 'fr' ? giftSettings.modal_text_fr : giftSettings.modal_text_en
  }, [giftSettings, lang, t])
